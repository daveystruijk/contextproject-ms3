...
/**
 * -MS3- This method is originally from TarsosDSP, but there are a few additions that were needed.
 * The additions will be commented and are recognized by the comments within '-MS3-'. -MS3-
 * 
 * <p>
 * A complex Domain Method onset detection function
 * 
 * <p>
 * Christopher Duxbury, Mike E. Davies, and Mark B. Sandler. Complex domain onset detection for
 * musical signals. In Proceedings of the Digital Audio Effects Conference, DAFx-03, pages 90-93,
 * London, UK, 2003
 * 
 * <p>
 * The implementation is a translation of onset.c from Aubio, Copyright (C) 2003-2009 Paul Brossier
 * piem@aubio.org
 * 
 * @author Joren Six
...
public class ComplexOnsetDetector implements AudioProcessor, OnsetDetector {
  ...
  /**
   * -MS3- Boolean to detect the first onset. Set to false if it is detected. -MS3-
   */
   ...
   * 
   * @param fftSize
   *          The size of the fft to take (e.g. 512)
   * @param peakThreshold
   *          A threshold used for peak picking. Values between 0.1 and 0.8. Default is 0.3, if too
   *          many onsets are detected adjust to 0.4 or 0.5.
   * @param silenceThreshold
   *          The threshold that defines when a buffer is silent. Default is -70dBSPL. -90 is also
   *          used.
   * @param minimumInterOnsetInterval
   *          The minimum inter-onset-interval in seconds. When two onsets are detected within this
   *          interval the last one does not count. Default is 0.004 seconds.
   */
  ...
  private void onsetDetection(AudioEvent audioEvent) {
    ...
    for (int j = 0; j < power.length; j++) {
      ...
      onsetValue += Math.sqrt(Math.abs(Math.pow(oldmag[j], 2) + Math.pow(power[j], 2) - 2.
          * oldmag[j] * power[j] * Math.cos(dev1[j] - phase[j])));
    ...
    /*
     * -MS3- This is were the first onset is initialized. It uses the boolean that checks if there
     * is an onset and the boolean that is initially on true to be sure this only gets called once.
     * -MS3-
     */
  ...
  /**
   * -MS3- Get the first onset of the track. -MS3-
   * 
   * @return first onset.
   */
